const otpStore = require('../config/redis');
const OTP_EXPIRY = 300; // 5 min

exports.generateOTP = async (phoneNumber) => {
    const otp = Math.floor(100000 + Math.random() * 900000).toString();

    otpStore.set(`otp:${phoneNumber}`, otp, OTP_EXPIRY);

    return otp;
};

exports.verifyOTP = async (phoneNumber, otp) => {
    const storedOtp = otpStore.get(`otp:${phoneNumber}`);
    ...
    if (isValid) {
        otpStore.delete(`otp:${phoneNumber}`);
    } else {
